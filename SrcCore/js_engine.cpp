...
ims_static std::vector<reg_function> s_js_ifs_object;

void js_add_reg_func(reg_function f)
{
	s_js_ifs_object.emplace_back(f);
}
...
struct js_aifs_block
{
	...
	{
		int64_t number;
		if (js_get_int64(m_ctx, &number, v)) {
			if (n) {
				n[0] = number;
				n[1] = 1;
			}
