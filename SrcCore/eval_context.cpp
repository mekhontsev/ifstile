...
const ims_val* eval_context::get_ast_val(const ast_context& p)
{
...
const ims_val* eval_context::eval_ref(size_t idx, bool is_geom)
{
	...
	if (!rf->ready[v_idx]) {
		...
		rf = &get_ref4(idx);//eval above may break the link, restore
		rf->in_eval = false;
...
const ims_val* eval_context::eval_pow(ast_context p, bool is_geom)
{
	...
	if (e_type == ESUBTYPE::integer) {
		...
		ims_error("pow: invalid exponent");
		return nullptr;
	}
...
const ims_val* eval_context::eval_call(ast_context p, bool is_geom)
{
	...
	if (!fb) {
		if (!is_geom)return get_ast_val(p);
		revert = true;
		ims_error("attempt to call invalid object");
		return nullptr;
	}
...
const ims_val* eval_context::eval_condition(ast_context p, bool is_geom)
{
	...
	for (size_t i = 0; i < na; i += 2) {
		...
		if (!a) {
			if (!is_geom)return get_ast_val(p);
			ims_error("if(): invalid argument {}", i);
			return nullptr;
		}
...
const ims_val* eval_context::eval_call_built_in(ast_context p, bool)
{
	...
	case BUILTIN_FUNC::arg:
	{
		double x, y;
		if (!a0->to_real(x)) {
			ims_error("arg: invalid argument 1");
			return nullptr;
		}

		pool_ptr a1(eval7(p.index(1), true));
		if (!a1 || !a1->to_real(y)) {
			ims_error("arg: invalid argument 2");
			return nullptr;
		}
	...
	default:
	{
		...
		if (!a0->to_real(x)) {
			...
			ims_error("{}: invalid argument", bf.name);
			return nullptr;
		}
...
const ims_val* eval_context::eval_diagonal(ast_context p, bool)
{
	...
	if (!vec || vec->num_vec_length() != n) {
		ims_error("$diagonal: invalid argument");
		return nullptr;
	}
...
const ims_val* eval_context::eval_charpoly(ast_context p, bool)
{
	pool_ptr a0(eval7(p.index(0), true));
	if (!a0) {
		ims_error("$charpoly: invalid argument");
		return nullptr;
	}

	pool_ptr v(eval_helpers::to_affine3(a0.get(), p.a->get_dim()));
	if (!v) {
		ims_error("$charpoly: argument is not affine");
	}
...
const ims_val* eval_context::eval_csg(ast_context p, bool is_geom)
{
	...
	for (size_t j = 0; j < na; ++j) {
		...
		if (err) {
			ims_error("csg: invalid argument {}", j + 1);
			return nullptr;
		}
	}
...
const ims_val* eval_context::eval_companion(ast_context p, bool)
{
	...
	for (size_t j = 0; j < na; ++j) {
		...
		if (!da[j]) {
			ims_error("$companion: invalid argument {}", j);
			return nullptr;
		}
	}
	...
	for (size_t i = 0; i < na; ++i) {
		...
		{
			ims_error("$companion arguments must be vectors");
			return nullptr;
		}
...
const ims_val* eval_context::eval_index(ast_context p, bool is_geom)
{
	...
	if (op.tt == ETYPE::index_imm) {
		...
		for (size_t i = 1; i < na; ++i) {
			...
			if (!ai || !ai->to_int(idx) || idx < 0) {
				if (!is_geom)return get_ast_val(p);
				ims_error("Invalid array index {} type {}", i - 1, (ai? (int)ai->gt():0));
				return nullptr;
			}
	...
	//which element to index, will change in the loop below
	pool_ptr v(eval7(p.index(0), is_geom));

	for (size_t j = 0; j < idx_arr.size(); ++j) {
		let idx = idx_arr[j];

		if (!v) {
			if (!is_geom)return get_ast_val(p);
			ims_error("Attempt to index invalid object");
			return  nullptr;
		}

		bool do_next_iter = false;
		if (!is_geom && v->is(ims_val::ETP::ast_ptr)) {
			...
			while (ast->h.tt == ETYPE::reference) {
				...
				//calculate in any case to enumerate dependencies!
				eval_ref(vidx, is_geom);
				ast = &get_ref4(vidx).c;
				if (++num > m_refs5.size()) {
					if (!is_geom)return get_ast_val(p);
					ims_error("Invalid recursive definition");
					return nullptr;
				}
			}
		...
		if (!v->is(ims_val::ETP::vector)) {
			if (!is_geom)return get_ast_val(p);
			ims_error("Attempt to index a non-vector object: {}", (size_t)v->gt());
			return  nullptr;
		}

		if (idx >= v->get_size()) {
			if (!is_geom)return get_ast_val(p);
			ims_error("Subscript out of range: {}, {}", idx, v->get_size());
			return  nullptr;
		}
