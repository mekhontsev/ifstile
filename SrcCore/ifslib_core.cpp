...
int32_t* ifslib_core::get_neighbor_graph()
{
	...
	for (let& e : m_neighbour_graph.m_edges) {
		...
		*d++ = m.er == ims_max ? -1 : static_cast<int32_t>(m.er);
		*d++ = m.ef == ims_max ? -1 : static_cast<int32_t>(m.ef);
	}
	return m_ret_int_array.data();
}

double* ifslib_core::get_map_rational(int32_t map_idx)
{
	if (!block_valid()) {
		return nullptr;
	}
	if (map_idx < 0 || (size_t)map_idx >= m_bi.m_em.size()) {
		std::cerr << "Map index " << map_idx << " is out of range. Valid range is 0 to " << (m_bi.m_em.size() - 1) << "." << std::endl;
		return nullptr;
	}
	let* m = m_bi.m_em[map_idx].ma.get();
	if (!m) {
		std::cerr << "Rational map data is not available for map index " << map_idx << "." << std::endl;
		return nullptr;
	};

	let num_el = m->affine_num_elems();

	m_ret_real_array.resize(1 + num_el * 2);
	auto* r = m_ret_real_array.data();
	*r++ = static_cast<int32_t>(m->extent(0));//dimension
	for (size_t i = 0; i < num_el; ++i) {
		let v = m->p_i(i);
		let nv = numerator(v);
		let dv = denominator(v);
		r[0] = static_cast<double>(nv);
		r[1] = static_cast<double>(dv);
		if (static_cast<int64_t>(r[0]) != nv || static_cast<int64_t>(r[1]) != dv) {
			return nullptr;
		}
		r += 2;
	}
	return m_ret_real_array.data();
}
