...
size_t neighbors_data::get_neighbor_graph(
	ims_graph_base& dst,
	std::vector<neighbor_edge_map>& maps,
	const ims_graph_base& dig) const
{
	maps.clear();
	dst.clear_base();
	...
	for (size_t s = 0; s < dig.num_ver(); ++s) {
		...
		for (size_t i = 0; i < sne; ++i) {
			...
			for (size_t j = 0; j < sne; ++j) {
				...
				let map_idx = maps.size();
				auto& m = maps.emplace_back();
				m.f = qj.m;
				m.r = qi.m;
	...
	for (size_t v = 0; v < nv; ++v) {
		...
		for (size_t i = 0; i < ne; ++i) {
			...
			for (size_t j = 0; j < ne2; ++j) {
				...
				neighbor_edge_map jm = em;
				if (!jm.join(maps[e2.m])) {
			...
			for (size_t j = 0; j < ne2; ++j) {
				...
				neighbor_edge_map jm = em;
				ASSUME(jm.join(maps[e2.m]));

				let map_idx = maps.size();
				maps.emplace_back() = jm;
	...
	//remap remaining vertices
	m_idxs.resize(dst.num_ver());
	size_t idx = 0;
	for(size_t v = 0; v < dst.num_ver(); ++v) {
		if (!m_visited[v]) {
			continue;
		}
		m_idxs[v] = idx++;		
